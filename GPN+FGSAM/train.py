...
def meta_learning_n_times(dataset_name:str, log_path:str, config:dict, cuda_idx=-1):
    ...
    def meta_learning(results=None, idx_repreat=0):
        ...
        optim = SAM(
            ...
            rho=config['rho'], lr=config['lr'], weight_decay=config['wd'], betas=config['betas'])
        ...
        def train_task(class_list:list, class_dict:dict, train=True):
            ...
            if train:
                ...
                loss_perturbed = forward(use_gnn=False)[0]
                (config['lam'] * loss_perturbed + (1 - config['lam']) * loss_origin).backward()
        ...
        for episode in tqdm(range(config['num_episodes']), ncols=70):
            # train
            # because of early-stop, no need to generate all the tasks at once
        ...
        write_file = {'results': results[dataset_name], 'config': config}
...
if __name__ == "__main__":
    ...
    # for dataset_name in ['corafull']:
    # for dataset_name in ['corafull', 'dblp']:
    # for dataset_name in ['corafull', 'dblp', 'ogbn-arxiv', 'amazon-clothing', 'amazon-electronics']:
    for dataset_name in ['corafull', 'dblp', 'ogbn-arxiv']:
        ...
        for n_way in [5]:
            ...
            for k_spt in [3]:
                ...
                # with open(f'log/grid_search/log_{folder_name}_{dataset_name}_{n_way}-way_{k_spt}-shot_{config["num_avail"]}-avail.json', 'r') as f:
                #     config = json.load(f)['config']

                meta_learning_n_times(dataset_name, 'log_layer%s_hdim%d/' % (config['num_layers'], config['hidden_dim']), config)