...
def meta_learning_n_times(dataset_name:str, log_path:str, config:dict, cuda_idx=-1):
    ...
    def meta_learning(results=None, idx_repreat=0):
        ...
        optim = FGSAMp(
            params=[{'params': gnn.parameters()}, {'params': classifier.parameters()}], 
        ...
        def train_task(train_iter:int, class_list:list, class_dict:dict, train=True):
            # sample task
    ...
    for dataset_name in ['corafull', 'dblp', 'ogbn-arxiv']:
        for n_way in [5, 10]:
            for k_spt in [3, 5]:
                ...
                config['n_way'] = n_way
                config['k_spt'] = k_spt
                meta_learning_n_times(dataset_name, 'log_k%d/' % (config['k']), config)