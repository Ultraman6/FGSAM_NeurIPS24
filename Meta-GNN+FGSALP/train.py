        ...
        def train_task(class_list:list, class_dict:dict, train=True):
            ...
            def forward(use_gnn=True):
                ...
                for j in range(config['finetune_steps']):
                    ...
                    loss_inner = loss_f(classifier(ori_emb, [w, b]), support_labels)
                    grad = torch.autograd.grad(loss_inner, [gc1_w, gc1_b, gc2_w, gc2_b, w, b])
            ...
            if train:
                if train_iter % config.get('k', 2) == 0:
                    gc1_w, gc1_b, gc2_w, gc2_b, w, b = gnn.gc1.w, gnn.gc1.b, gnn.gc2.w, gnn.gc2.b, classifier.w, classifier.b
                    loss_mlp, _ = forward(use_gnn=False)
                    g_mlp = torch.autograd.grad(loss_mlp, [gc1_w, gc1_b, gc2_w, gc2_b, w, b])
                    g_mlp = [g_mlp[:4], g_mlp[4:]]
                    loss_origin.backward(retain_graph=True)
                else:
                    g_mlp = None
                    loss_origin.backward()
                optim.step(train_iter, forward, g_mlp, zero_grad=True)
