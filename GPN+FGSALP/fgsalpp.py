...
class FGSALPp(torch.optim.Optimizer):
    def __init__(self, params, base_optimizer, k:int, alpha:float, rho:float=0.05, lam:float=0.5,
                 num_pert:int=-1, topology_norm:bool=False, rho_min:float=0.01, rho_max:float=1.0, rho_lr:float=1.0, **kwargs):
        ...
        if t % self.k == 0:
            ...
            for i, group in enumerate(self.param_groups[:self.num_pert]):
                ...
                for j, p in enumerate(group['params']):
                    ...
                    self.state[p]['old_p_grad_mlp'] = g_mlp[i][j]
                    with torch.no_grad():
        ...
        for group in self.param_groups[:self.num_pert]:
            for p in group['params']:
                if p.grad is None:
                    continue
                if t % self.k == 0:
                    old_p_grad_mlp = self.state[p]['old_p_grad_mlp']
                    g_mlp_grad_norm = FGSALPp.normalized(old_p_grad_mlp)
                    gs_grad_norm = FGSALPp.normalized(p.grad)
                    self.state[p]['gv'] = torch.sub(p.grad, p.grad.norm(p=2) * torch.sum(
                        g_mlp_grad_norm * gs_grad_norm) * g_mlp_grad_norm)
                else:
                    with torch.no_grad():
                        gv = self.state[p]['gv']
