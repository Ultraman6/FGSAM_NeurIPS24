...
def meta_learning_n_times(dataset_name:str, log_path:str, config:dict, cuda_idx=-1):
    ...
    def meta_learning(results=None, idx_repreat=0):
        ...
        adj = u.edge_index_to_adj_with_rw_norm(edge_index, num_nodes)
        ...
        if config.get('gpn', 'linear') == 'linear':
        ...
        optim = FGSALPp(
            [{'params': encoder.parameters()}, {'params': scorer.parameters()}], base_optim,
            k=config.get('k', 2), alpha=config.get('alpha', 0.7), rho=config['rho'], lam=config['lam'],
            lr=config['lr'], weight_decay=config['wd'], betas=config['betas'],
            rho_min=config['rho_min'], rho_max=config['rho_max'], rho_lr=config['rho_lr'])
        ...
        def train_task(train_index, class_list:list, class_dict:dict, train=True):
            ...
            def forward(use_gnn=True):
                ...
                support_scores = torch.sigmoid(support_degrees * support_scores)
                support_scores = torch.softmax(support_scores, dim=-1).unsqueeze(-1)
                support_embeddings = support_embeddings * support_scores

                prototype_embeddings = support_embeddings.mean(dim=1)
                logits = -torch.cdist(query_embeddings, prototype_embeddings)
            ...
            loss_origin, acc = forward(use_gnn=True)
            if train:
                if train_index % config.get('k', 2) == 0:
                    ...
                    # split grads into two groups matching param_groups
                    num_enc = len(list(encoder.parameters()))
                    g_mlp = [g_mlp[:num_enc], g_mlp[num_enc:]]
                    loss_origin.backward(retain_graph=True)
                else:
                    g_mlp = None
        ...
        for episode in tqdm(range(config['num_episodes']), ncols=70):
            ...
            if (episode + 1) % config['show_train_interval'] == 0:
                rho_stats = optim.get_rho_stats()
                print('Train #%d | acc: %.4f | rho[m/ss/min/max]=[%.4f/%.4f/%.4f/%.4f]' % (
                    (episode + 1), np.mean(acc_meta_train, axis=0), rho_stats['mean'], rho_stats['std'], rho_stats['min'], rho_stats['max']))
    ...
    for dataset_name in ['corafull', 'dblp', 'ogbn-arxiv']:
        for n_way in [5, 10]:
            for k_spt in [3, 5]:
                ...
                config['n_way'] = n_way
                config['k_spt'] = k_spt
                meta_learning_n_times(dataset_name, 'log_%s_layer%s_hdim%d/' % (config['gpn'], config['num_layers'], config['hidden_dim']), config)
