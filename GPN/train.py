...
def meta_learning_n_times(dataset_name:str, log_path:str, config:dict, cuda_idx=-1):
    ...
    def meta_learning(results=None, idx_repreat=0):
        ...
        def train_task(class_list:list, class_dict:dict, train=True):
            ...
            support_scores = scores[idx_spt].view([n_way, k_spt])
    ...
    for dataset_name in ['corafull', 'dblp', 'ogbn-arxiv']:
        for n_way in [5, 10]:
            for k_spt in [3, 5]:
                ...
                config['n_way'] = n_way
                config['k_spt'] = k_spt
                meta_learning_n_times(dataset_name, 'log_%s_layer_%s_hdim_%d/' % (
                    config['gpn'], config['num_layers'], config['hidden_dim']), config)